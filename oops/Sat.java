...
/*
// P2 create a class box with h,w,d as the member variables and volume as a method and 3 different constructors
class Box 
{    int h, w, d;

    Box()                        {   h = 10;   w = 5;  d = 9;  }
    Box(int x, int y, int z)     { h = x;  w = y;  d = z;    }
    Box(Box ob)                  { }
    int v()                      { return (h * w * d);    }
}
class Sat
...
//P3  Wap to create a student class with the following members: rollno,rank,name, percentage,attendence
// wa constructor and a method that displays student details and a method which checks the standards(meet standard)
/*import java.util.Scanner;

class A {
    int roll, rank;
    double percentage, attendence;
    String name;

    void readdata() // method that reads data of user
    {
        Scanner ob = new Scanner(System.in);
        name = ob.nextLine();
        roll = ob.nextInt();
        percentage = ob.nextDouble();
        rank = ob.nextInt();
        attendence = ob.nextDouble();
    }
    ...
    void standards(double percentage) // method that checks elibibiliy
    {
        if (percentage > 60.00) {  System.out.println("Candidete's percentage is elligible ");  }
         else                  { System.out.println("The candidate's  is not eligible");    }
    }

    void standards(int attendence) // method that checks elibibiliy
    {
        if (attendence > 80.00) {  System.out.println("Candidete's attendence is elligible ");}
         else {System.out.println("The candidate's attendence is not eligible");} 
    }  }

class Sat { // main method
    public static void main(String args[]) {
        A ob1 = new A();
        ob1.readdata();
        ob1.display();
        ob1.standards(ob1.percentage);
    ...
    void mul() {
        if (r2 == c1) {
            ...
            for (int i = 0; i < r1; i++)
                for (int j = 0; j < c2; j++) {
                    d[i][j] = 0;
         d           for (int k = 0; k < r1 * c2; k++)
                        d[i][j] = a[i][k] * b[k][j];
                }
            System.out.println("The product matrix of the 2 matrices is");
            for (int i = 0; i < r1; i++)
                for (int j = 0; j < c2; j++)
...
/*
//Passing an object as a parameter and returning obj example:
import java.util.Scanner;
class a
{ int i,j;
    a(int x,int y){ i=x;j=y; }     //constructor

   boolean checkEquality(a obj)      // passing obj
    { if((i==obj.i) && (j==obj.j)) {return true;} else {return false;} }

   a returningObj()                    //returning object
   {  a ob5=new a(i+10,j+10);  return(ob5);       }
}
class Sat 
{
    public static void main(String args[]) 
    {
        Scanner ob=new Scanner(System.in);
        a ob1=new a(ob.nextInt(),ob.nextInt());
        a ob2=new a(ob.nextInt(),ob.nextInt());
        a ob3=new a(ob.nextInt(),ob.nextInt());
        a ob4=ob1.returningObj();
        
        System.out.println(ob4.i+" "+ob4.j);
        System.out.println("Object 1's variables are equal to object 2 :"+ob1.checkEquality(ob2));
        System.out.println("Object 3's variables are equal to object 2 :"+ob2.checkEquality(ob3));
    }   
}*/
/*
...
{    static int y=2,z; //static variables
    ...
    static void Method(int x) {System.out.println(x);System.out.println(y);System.out.println(z);} //static method

    static {System.out.println("This is Static block");  z=y*20;}  //static block is first executed
}
class Sat
{ public static void main(String args[]) 
    {    classEg.Method(88); }           } */
...
{ Sub(int x,int y,int z)  {  i=x;j=y;k=z;  }}
...
/*class SuperC
{ int i,j,k;
  SuperC(int x,int y,int z)  {  i=x;j=y;z=k; }
  int method() {return(12);}
}

class SubC extends SuperC
{  int l;
   SubC(int x,int y,int z, int m)  {super(x,y,z);  l=m; } 
   //super must be the first line of the subclass constructor

   int SubMethod()  {return(super.i+super.method());}
}
...
class Sat
{ public static void main(String args[]) 
   {  A ob=new A();     B ob1=new B();      C ob2=new C();       //3 objects for 3 classes

      A ref;    //creating a refference of type A

      ref=ob;  ref.method();         //ref reffering to A's obj 
      ref=ob1;  ref.method();        //ref reffering to B's obj
      ref=ob2;  ref.method();        //ref reffering to C's obj
}  }*/

//Example 2 on Dynamic method dispatch   //important  topic this is as runtime polymorphism is used.
/*class figure
{  int a,b;  figure(int i, int j) {a=i;b=j;}  double area() {return(a*b);} }

class rectangle extends figure

{  rectangle()   {super(1,2);}        double area()  {return(a*b);}   }

class square extends figure

{  square() {super(6,8);}              double area()  {return(a*b);}    }

class parallelogram extends figure

{  parallelogram() {super(8,9);}       double area()  {return(a*b);}    }
...
/*abstract class a
{
   ...
   void method4()         {System.out.println("Normal method in super class getting overridden");}
}
class b extends a
{  void method1() {System.out.println("Abstract method implementation by sub class");}
   void method2(int x) {System.out.println("Abstract method2 "+x);}
   void method3(int x,int y) {System.out.println("Abstract method3 "+x+y);}
   
   void method4()         {System.out.println("Normal method in subclass getting overridden");}
}
class Sat
{public static void main(String args[])
    { b ob=new b();    ob.method1(); ob.method2(9); ob.method3(1,2); ob.method4();    }} 

//Example 2 for Abstract classes
/*Create an Abstract class Figure with required members.
 Create two subclasses of Figure – Rectangle, Triangle with required members. 
 In main method create the objects of Rectangle and Triangle class and call the methods to calculate the area.
Wap making use of Figure- interface, Rectangle -class, Triangle-class that illustrates dynamic polymorphism using interface*/
/*import java.util.Scanner;
...
{   triangle() {super();}
    public double area() {return(x*y*0.5);}
}
class Sat
{ public static void main(String args[])
 {  
...
//a concrete method with eligibility criteria and 3 abstract methods :writtentest(3), interview(), package
...
{ tcs(String y,int x) {super(y,x);}   //constructor
  int writtenTest(double a,double b, double c) {if ((a+b+c)>200.00) { OverallGrade=(a+b+c)/500; return(1);}   else {return(0);}  }
  int interview(double a,double b) {if ((a+b)>180) {  OverallGrade+=(a+b)/5; return(1);}   else {return(0);}}
  void Package()
   {  if (OverallGrade>9) {System.out.println("Your Salery is 25 lakhs/anum");}
      if (OverallGrade<=9 && OverallGrade>8.5) {System.out.println("Your Salery is 15 lakhs/anum");} 
      if (OverallGrade<=8.5 ) {System.out.println("Your Salery is 10 lakhs/anum");} 
   }
}

class acenture extends Student
{ acenture(String y,int x) {super(y,x);}  //constructor
   int writtenTest(double a,double b, double c) {if ((a+b+c)>250.00) { OverallGrade=(a+b+c)/500; return(1);}   else {return(0);}  }
   int interview(double a,double b) {if ((a+b)>160) {  OverallGrade+=(a+b)/5; return(1);}   else {return(0);}}
   void Package()
   {  if (OverallGrade>9.5) {System.out.println("Your Salery is 30 lakhs/anum");}
      if (OverallGrade<=9.5 && OverallGrade>8) {System.out.println("Your Salery is 22 lakhs/anum");} 
      if (OverallGrade<=8) {System.out.println("Your Salery is 14 lakhs/anum");} 
   }
}
...
{   int salery;String designation;
    FullTimeEmployee() 
        {super();Scanner ob = new Scanner(System.in);
            System.out.println("Enter designation and salery");
        designation = ob.nextLine(); salery = ob.nextInt();}
...
{   int workingHours,ratePerHour;
    PartTimeEmployee() 
        ...
        workingHours = ob.nextInt(); ratePerHour = ob.nextInt();}
    int calculatePay(){return(workingHours*ratePerHour);}
    void display() {System.out.println("Name: "+name+"\n Age:"+age+"\n Gender: "+gender+"\n Address");
...
*//*
   ...
   {int r;Double t;
     circle()   
     {   Scanner ob = new Scanner(System.in);
         System.out.println("Enter radius of circle"); r=ob.nextInt();
         System.out.println("Enter angle  of arc"); t=ob.nextDouble(); t=t*3.14/180;
      }
     void circleCircumference(){System.out.println("Circumference of circle is: "+2*3.14*r);}
     void arclength(){System.out.println("arc length  is: "+r*t);}}
   class Sat
   { public static void main(String args[])
   {  
...
{ private static A ob=new A();  //this creates object during class loading time
  ...
  public static A objSenderMethod() {return(ob);} //return type is A as it returnss object
  
  public void NormalMethod() {System.out.println("Yo!sup U accessed singleton class method ");}
}
class Sat
{
    public static void main(String args[])
